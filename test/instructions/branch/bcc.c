...
/**
 * Tests if the Carry flag is clear.
 * Only the PC should be affected.
 */

/** There is one test which will perform 3 BCC instructions
 * 1. BPL 0x0F (with Carry flag set)
 * 2. BPL 0x0F (with Carry flag clear)
 * 3. BPL 0xFE (with Carry flag clear)
 */
...
#define REL_TEST                                                                 \
  {                                                                              \
    writePS(&cpu, CM);                                                           \
    writeMemory(&cpu, 0x0200, INS_BCC_REL);                                      \
    ...
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(0x0202, readPC(&cpu),                        \
                                    "PC is not set right for the first test.");  \
                                                                                 \
    writePS(&cpu, 0x00);                                                         \
    writeMemory(&cpu, 0x0202, INS_BCC_REL);                                      \
    ...
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(0x0213, readPC(&cpu),                        \
                                    "PC is not set right for the second test."); \
                                                                                 \
    writePS(&cpu, 0x00);                                                         \
    writeMemory(&cpu, 0x0213, INS_BCC_REL);                                      \
