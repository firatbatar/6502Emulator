...
/** There is one test which will perform 3 BCS instructions
 * 1. BPL 0x0F (with Carry flag clear)
 * 2. BPL 0x0F (with Carry flag set)
 * 3. BPL 0xFE (with Carry flag set)
 */
...
#define REL_TEST                                                                 \
  {                                                                              \
    writePS(&cpu, 0x00);                                                         \
    writeMemory(&cpu, 0x0200, INS_BCS_REL);                                      \
    ...
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(0x0202, readPC(&cpu),                        \
                                    "PC is not set right for the first test.");  \
                                                                                 \
    writePS(&cpu, CM);                                                           \
    writeMemory(&cpu, 0x0202, INS_BCS_REL);                                      \
    ...
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(0x0213, readPC(&cpu),                        \
                                    "PC is not set right for the second test."); \
                                                                                 \
    writePS(&cpu, CM);                                                           \
    writeMemory(&cpu, 0x0213, INS_BCS_REL);                                      \
